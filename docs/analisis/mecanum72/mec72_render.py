...
# ---- 3: despiece axial ----
clear(); M = mats(); anchors = {}; allpts = []
OFF = {'placa_A': -0.030, 'placa_B': 0.030, 'rodillo': 0.0, 'pasador': 0.0,
       'rodamiento_A': -0.058, 'rodamiento_B': 0.058,
       'casquillo_A': -0.082, 'casquillo_B': 0.082,
       'polea': -0.108, 'retenedor': 0.108, 'eje': 0.0, 'perno_c': -0.030}
for n, m in izq.items():
    off = 0.0
    for k, v in OFF.items():
        if n.startswith(k): off = v
    vtx = (m.vertices + np.array([0, 0, off])) @ Rz2x.T
    add_mesh(n, vtx, m.faces, matfor(n, M))
    allpts += [vtx.min(0), vtx.max(0)]
    key = None
    if n == 'placa_A': key = 'Placa A — PA-CF\n(corona de 6 dientes + asiento 6804)'
    if n == 'placa_B': key = 'Placa B — PA-CF\n(bolsillos de corona + asiento 6804)'
    if n == 'rodillo_2': key = 'Rodillo ×6 — 33.5 × Ø18/Ø13\n(los tuyos) a 46°'
    if n == 'pasador_1': key = 'Pasador Ø3.2×43 ×6 (1/8")'
    if n == 'rodamiento_A': key = 'Rodamiento 6804 ×2\n(20×32×7, uno por cara)'
    if n == 'casquillo_A': key = 'Casquillo 1/2"→20 ×2\n(PA-CF, brida de tope)'
    if n == 'polea_48T': key = 'Polea GT2 48T ×1\n(correa 6 mm, PD 30.56)'
    if n == 'retenedor': key = 'Retenedor ×1\n(misma brida, sin dientes)'
    if n == 'eje_media': key = 'Eje pasante Ø1/2" (12.7)\nredondo, fijo'
    if key:
        c = (vtx.min(0) + vtx.max(0)) / 2
        anchors[key] = [float(c[0]), float(c[1]), float(vtx[:, 2].max())]
lights()
cam = camera((0.05, -0.40, 0.15), (0, 0, 0.004), 50)
fit(cam, (0, 0, 0.004), allpts)
render(p('m72_exploded.png'), 2000, 1100, 64)
for k in list(anchors):
    sc = bpy.context.scene
    uv = world_to_camera_view(sc, cam, Vector(anchors[k]))
