...
gs = gridspec.GridSpec(3, 3, height_ratios=[0.95, 1.15, 0.95],
...
ax.text(0.5, -0.05, 'Brida Ø48 atornillada al cubo con 6 tornillos 2.9×6 ·\n'
...
ax.text(0.5, -0.05, 'Asiento Ø32.15×7 con hombro · casquillo PA-CF 12.7→20 ·\n'
...
arriba = [it for i, it in enumerate(items) if i % 2 == 0]
abajo = [it for i, it in enumerate(items) if i % 2 == 1]
for grupo, base, paso in ((arriba, -0.09, -0.115), (abajo, 1.09, 0.115)):
    for j, (label, (px, py)) in enumerate(grupo):
        ty = (base + (j % 2) * paso) * H
        tx = (j + 0.5) / len(grupo) * W
        ax.annotate(label, xy=(px, py), xytext=(tx, ty), fontsize=8.5,
                    ha='center', va='center', bbox=BOX,
                    arrowprops=dict(arrowstyle='->', color='#333', lw=0.9, shrinkB=4),
...
try:
    import re
    import subprocess
    cola = subprocess.run(['tail', '-c', '400000',
                           p('MECANUM72_cama_PACF_100.gcode')],
                          capture_output=True).stdout.decode('utf8', 'replace')
    m = re.search(r'estimated printing time \(normal mode\) = (.+)', cola)
    tiempo = m.group(1).strip() if m else '—'
    m2 = re.search(r'total filament used \[g\] = ([\d.]+)', cola)
    gramos = m2.group(1) if m2 else '53'
except Exception:
    tiempo, gramos = '—', '53'
imp = (
...
fig.subplots_adjust(top=0.945, bottom=0.02, left=0.02, right=0.99, hspace=0.34, wspace=0.06)
