...
def build(hand='izq'):
    ...
    asm = cq.Assembly(name=f"Mecanum64v8_{hand}")
...
if __name__ == '__main__':
    for hand in ('izq', 'der'):
        ...
        for n, s in [(f'm64v8_placa_A_{hand}', pa), (f'm64v8_placa_B_{hand}', pb)]:
        ...
        asm.save(os.path.join(OUT, f'm64v8_ensamble_{hand}.step'))
        print(f'ensamble {hand} exportado')
    cq.exporters.export(rodillo_solid(), os.path.join(OUT, 'm64v8_rodillo.step'))
    cq.exporters.export(rodillo_solid(), os.path.join(OUT, 'm64v8_rodillo.stl'),
                        tolerance=0.03, angularTolerance=0.25)
    print('done')
