...
for fn in sorted(glob.glob(p('bo4_*.stl'))):
...
for h in ('izq', 'der'):
    tm2 = trimesh.load(p(f'm64v7_{h}.glb'))
    partes = []
...
mot.apply_translation([1196.0, 2.4 * 0 - 2.4, 186.9])   # recentrar al centro motor
spo.apply_translation([1196.0, -2.4, 218.4])
Rzp3 = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]], float)
Rz180 = np.diag([-1.0, -1.0, 1.0])
# der: cuerpo hacia -Y (Rzp3); izq: cara hacia -Y (Rz180@Rzp3)
for h, ys in (('der', G['der'][0]), ('izq', G['izq'][0])):
    if h == 'der':
        R, ymc = Rzp3, ys - 31.5
    else:
        R, ymc = Rz180 @ Rzp3, ys + 31.5
    add(f'motor_blk_{h}', mot, [52, 110, 62, 255], T_from(R, (X_MOTOR[h], ymc, MZ)))
    add(f'spool_blk_{h}', spo, COL['polea'], T_from(R, (X_MOTOR[h], ys, MZ)))
...
for h in ('der', 'izq'):
    g1, g2 = G[h]
    xm = X_MOTOR[h]
    c1, c2 = [X_EJES[k] for k in KS[h][1:3]]
    # lazo del motor (garganta 1): motor -> bajo las 2 poleas centrales
    cinta(f'correa_{h}_m', [(xm - 31.5, MZ), (c1, Z_EJE - r_p - 1.5),
                            (c2, Z_EJE - r_p - 1.5), (xm + 31.5, MZ)], g1)
    # transferencias (garganta 2): centrales -> extremos
    a, d = X_EJES[KS[h][0]], X_EJES[KS[h][3]]
    for (u, v) in ((a, c1), (c2, d)):
        for dz in (r_p - 2.0, -(r_p - 2.0)):
            cinta(f'correa_{h}_t{u:.0f}', [(u, Z_EJE + dz), (v, Z_EJE + dz)], g2)


esc.export(p('bloque_omni_zp_v4.glb'))
print('nodos totales:', len(esc.graph.nodes_geometry), '-> bloque_omni_zp_v4.glb')
