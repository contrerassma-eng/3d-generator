...
# ---------- el bloque (piezas v3.2 + ruedas y correas, como bo3_scene) ----------
NEJES, PASO = 8, 74.75
Z_EJE, W2 = 83.1, 18.3
...
X_IDLER = {c: [(X_EJES[a] + X_EJES[b]) / 2 for a, b in zip(KS[c][:-1], KS[c][1:])]
           for c in (-1, 1)}
...
for fn in sorted(glob.glob(p('bo3_*.stl'))):
...
for h in ('izq', 'der'):
    ...
    for k in range(NEJES):
        ...
        for j, y in enumerate(Y_RUEDAS):
            giro = trimesh.transformations.rotation_matrix(radians(90 * j + 15 * k), [0, 0, 1])
...
for cy in (-1, 1):
    R = Rzp3 if cy > 0 else Rz180 @ Rzp3
    zs = SPOOL_Z - MOT_DROP
    add(f'motor_blk_{cy}', mot, COLZP['UniDrive'],
        T_from(R, (X_MOTOR[cy], cy * (POLEA_Y - 31.5), zs)))
    add(f'spool_blk_{cy}', spo, COL['polea'],
...
for cy in (-1, 1):
    ...
    pts = [(X_MOTOR[cy] - r_sp, zs), (xs_p[0], Z_EJE - r_p - 1.5)]
    for xi, xp in zip(X_IDLER[cy], xs_p[1:]):
        pts.append((xi, IDLER_Z + r_i + 1.5))
        pts.append((xp, Z_EJE - r_p - 1.5))
    pts.append((X_MOTOR[cy] + r_sp, zs))
    cinta(f'correa_{cy}', pts, y)

esc.export(p('bloque_omni_zp.glb'))
print('nodos totales:', len(esc.graph.nodes_geometry), '-> bloque_omni_zp.glb')
