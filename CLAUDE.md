...
| "dispensador \<código\>" / "dosificador de alimento" / "bebedero de bidón" | Familia de aparatos mecánicos sobre bidón de 20 L (alimento dosificado por cajón de cavidad fija + agua de nivel constante). Parámetros en `projects/<X>/input/params.json`; `gen_dispensador.py` → piezas, `ens_dispensador.py --anim` → ensambles y ciclo, `verifica_dispensador.py` → gate G-DIS (14 pruebas), `doc_dispensador.py` → memoria y manuales, `web_dispensador.py` → visor. Detalle: `docs/DISPENSADOR.md`. |
| "simula \<código\>" / "simula la croqueta" / "grano a grano" | Gate **G-SIM**: `python pipeline/sim_granos.py projects/<X> todo --anim`. Simula el alimento grano a grano (DEM) dentro de las mallas reales para poner a prueba lo que G-DIS no puede: llenado real de la cavidad, si el cuello atasca, si el agitador sirve y si el bisel parte la croqueta. Calibra el grano contra ángulo de reposo y densidad aparente publicados ANTES de tocar el aparato. Es LENTO (decenas de minutos) y es capa `user`: un modelo, no una medición. Detalle: `docs/DISPENSADOR.md`. |
