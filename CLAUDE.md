...
| "módulo transfer omni bidireccional BF \<n\>" | `python pipeline/modulo_transfer.py projects/<X> [--bf --paso …]` → frame de canal plegado, ejes hex con omnis de avance (RUEDA-OMNI-58), omnis GIRADAS 90° en los huecos sobre ejes cortos movidas por o-rings desde el eje común inferior, 2 UniDrive, soportes FR8ZZ, travesaños y tapa ranurada. DISEÑO capa `user` con verificación propia (encaje BF, tangencias, envolvente ≥120°, apoyo de caja mínima, holguras, STL estancos). Decisiones en `projects/<X>/DECISIONES.md`. |
