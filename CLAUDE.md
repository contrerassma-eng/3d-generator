...
| "vista de frente de esta rueda omni" (foto + diámetro exterior) | `python pipeline/rueda_omni.py medir projects/<X> --diametro <mm>` y luego `vista`. Una sola foto NO da capa `measured`: el diámetro declarado es el único ancla de escala y todo lo demás sale por proporción, con `fuente` por cota. Lo no determinable (barreno, ancho) se queda SIN cota. Detalle: `docs/DIBUJO_TECNICO.md`. |
| "hazme las piezas de esa rueda omni" (hex, Ø eje, espesor de placa) | `python pipeline/rueda_omni_piezas.py projects/<X> --hex <mm> --eje <mm> --espesor <mm>` → STL de placas/rodillos/ejes, `conjunto.glb`, `placas.dxf` y `piezas.json`. Es DISEÑO (capa `user`) heredando cotas de la medición: la separación entre hileras y la circunferencia de tornillos las calcula el script, no se copian del original. Verifica interferencias y estanqueidad. Detalle: `docs/DIBUJO_TECNICO.md`. |
| "módulo transfer omni bidireccional BF \<n\>" | `python pipeline/modulo_transfer.py projects/<X> [--bf --paso …]` → planta COMPLETA cubierta por omnis: frame de canal plegado, ejes hex con omnis de avance (RUEDA-OMNI-58) hasta ambas almas, omnis GIRADAS 90° en los huecos sobre ejes cortos movidas por o-rings desde el eje común inferior, 2 UniDrive COLGADOS bajo el fondo en bancadas, soportes FR8ZZ, travesaños y tapa de ranuras estadio. DISEÑO capa `user` con verificación propia (encaje BF, tangencias, envolvente ≥120°, apoyo de caja mínima, holguras, STL estancos). Decisiones en `projects/<X>/DECISIONES.md`. |
