#!/usr/bin/env node
// planos_rodillos.mjs — PLANOS DE FABRICACIÓN del rodillo plano y del cónico.
//
// Emite, en `planos/`:
//   - planos_fabricacion_rc48.pdf : portada + lista de materiales + una lámina
...
//   - conjunto_rc48.dxf : vistas del conjunto a escala real
//
//   cd cad && node ensambles/rodillos_sch40/planos_rodillos.mjs [fecha]
...
const doc = JSON.parse(readFileSync(join(aquí, 'rodillos_sch40.json'), 'utf8'));
const outDir = join(aquí, 'planos');
mkdirSync(outDir, { recursive: true });
const fecha = process.argv[2] || '—';
const PROY = 'RC-SCH40-48 — RODILLO PLANO Y CÓNICO SOBRE CAÑERÍA SCH40 1-1/2\"';
const FUENTE = 'diseño paramétrico foto3d — capa user (escala 0.104712 mm/px, ver ESCALA.md)';
...
const desig = (n) => n.replace(/^(FIJO|MÓVIL|PLANO|CÓNICO)\s·\s/, '').replace(/\s*\([^)]*\)\s*$/, '').trim();
// Las piezas COMPRADAS se agrupan por designación + código de catálogo: dos
// tapas simétricas son el mismo artículo aunque su perfil esté espejado.
// Las FABRICADAS se agrupan por geometría, pero descartando los ids de las
// entidades del perfil (son un contador global, no parte de la forma).
const sinIds = (o) => JSON.stringify(o ?? {}).replace(/"id":"[^"]*",?/g, '');
const firma = (p) => (p.hardware || p.componente)
  ? `C|${desig(p.name)}|${p.componente || p.norma || ''}`
  : `F|${desig(p.name)}|${p.features.length}|${p.features.map(f => `${f.shape}${sinIds(f.params)}`).join(';')}`;

const grupos = new Map();
for (const p of doc.parts) {
  const k = firma(p);
  if (grupos.has(k)) grupos.get(k).cant++;
  else grupos.set(k, { name: p.name, desig: desig(p.name), part: p, cant: 1, capa: (p.name.match(/^(FIJO|MÓVIL|PLANO|CÓNICO)/) || [, '—'])[1] });
}
...
for (const g of lista) {
  ...
  // En este conjunto cada pieza declara su material en `extra.material`: se
  // respeta ese dato antes de caer en la inferencia por nombre.
  const material = comprada ? (g.part.componente || g.part.norma || 'componente de catálogo')
      : (g.part.material || materialDe(g.desig, g.part.chapa));
  let plano = '';
  if (fabricada) {
    planoN++;
    plano = `RC48-${String(planoN).padStart(2, '0')}`;
    const meta = {
...
function despieceSheets() {
  ...
  for (let i = 0; i < despiece.length; i += filas) {
    ...
    sh.titleBlock({
      ...
      escala: '—', fecha, numPlano: `RC48-LM${i / filas + 1}`,
    });
    hs.push(sh);
  }
  return hs;
...
try {
  ...
  const dxf = exportDrawingDXF(partes, {
    designacion: 'CONJUNTO — RODILLO PLANO Y CÓNICO', proyecto: PROY, fuente: FUENTE,
    piezas: String(partes.length), fecha, numPlano: 'RC48-CJ',
    nota: 'vistas del primer diedro a escala real (mm)',
  });
  writeFileSync(join(outDir, dxf.name || 'conjunto_rc48.dxf'), dxf.data ?? dxf);
} catch (err) {
